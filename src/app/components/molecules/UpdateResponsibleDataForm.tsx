...
import { updateStudentResponsibleCall } from '@/app/api/student'
import { api } from '@/app/api'

interface UpdateResponsibleFormValues {
  id: string
  name: string
  avatarUrl: string
  rg: string
...
export const UpdateResponsibleDataForm = forwardRef<
  HTMLFormElement,
  ResponsibleFormProps
>(({ studentId, responsibleId, onSuccess }, ref) => {
  const [initialValues, setInitialValues] =
    useState<UpdateResponsibleFormValues>({
      id: '',
      name: '',
      avatarUrl: '',
      rg: '',
  ...
  useEffect(() => {
    const fetchResponsible = async () => {
      try {
        ...
        const responsible = response.data.find(
          (r: UpdateResponsibleFormValues) => r.id === responsibleId
        )
        if (responsible) {
          setInitialValues({
            id: responsible.id,
            ...
            cpf: responsible.cpf,
            nis: responsible.nis,
            degree_of_kinship: responsible.degree_of_kinship,
            education_level: responsible.education_level,
            current_job: responsible.current_job,
            wage: responsible.wage,
            ethnicity: responsible.ethnicity,
            primary_phone: responsible.primary_phone,
            responsible_observations: responsible.responsible_observations
          })
        }
      } catch (error) {
        console.error('Failed to fetch responsible', error)
        toast({
          title: 'Erro',
          description:
            'Não foi possível buscar o responsável. Tente novamente mais tarde.',
          status: 'error',
          duration: 9000,
          isClosable: true
        })
    ...
    fetchResponsible()
  }, [studentId, responsibleId, toast])

  const formik = useFormik<UpdateResponsibleFormValues>({
    initialValues,
    ...
    onSubmit: async (values) => {
      try {
        if (responsibleId) {
          await updateStudentResponsibleCall(responsibleId, values)
          toast({
            title: 'Dados do responsável atualizados com sucesso!',
            status: 'success',
            duration: 9000,
            isClosable: true
          })
          ...
          throw new Error('ID do responsável está faltando')
        }
      } catch (error: any) {
        toast({
          title: 'Erro ao atualizar dados do responsável',
  ...
  const handleCurrencyChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFieldValue(
      name as keyof UpdateResponsibleFormValues,
