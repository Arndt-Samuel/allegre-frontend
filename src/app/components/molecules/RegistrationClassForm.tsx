...
import { createClassCall } from '@/app/api/class'
import { api } from '@/app/api'
import { useClassStore } from '@/app/hooks/classStore'
...
export const RegistrationClassForm = forwardRef<
  HTMLFormElement,
  ClassFormProps
>(({ onSubmit }, ref) => {
  ...
  const formik = useFormik<ClassFormValues>({
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (values.logoUrl) {
          formData.append('logo', values.logoUrl as File)
        }
        ...
        const newClass = await createClassCall(formData)
        addClass(newClass)
        onSubmit(values)
        toast({
      ...
      >
        <Flex flexDir={'column'} h={'23.62%'}>
          ...
          >
            ...
            {values.logoUrl ? (
              <Image
                src={URL.createObjectURL(values.logoUrl as File)}
